...
public:
  ...
  virtual int writeBuffer(const void* buffer, int samples, int size = 1);	// Scrivi un buffer a uno stream aperto di output
    ...
    std::string get_sound_error() const { return pa_strerror(sound_error); }	// Ritorna la stringa del precedente errore di PulseAudio


    int get_INstate() const { return INstate; }    // Ritorna il valore della variable INstate per lo stato dell'input
    int get_OUTstate() const { return OUTstate; }	// Ritorna il valore della variable OUTstate per lo stato dell'output
